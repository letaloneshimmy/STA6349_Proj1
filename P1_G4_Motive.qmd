...
<font color="DarkGreen">Fear</font> Prior's and Posteriors for Competition Motive Episodes:
------
...
<font color="DarkBlue">Joy</font>  Prior's and Posteriors for Competition Motive Episodes:
------
...
<font color="DarkRed">Ennui</font>  Prior's and Posteriors for Competition Motive Episodes:
------
...
Evolution of the posteriors of all 3 emotions in Competition Motive Episodes over the decades:
------
...
p <- ggplot(graph_data, aes(x = pie, y = f_of_pi, fill = emotion)) +
    ...
    labs(title = "Posterior Beliefs of emotions after {closest_state} episodes", x = "pi", y = "f(pi)", legend = "Emotion", subtitle = "Modelling the probability that a villian will be caught given that their motive was Competition", parse = TRUE) +
...
## Villian Motive Power: Fear Iteration
```{r}
...
```
## Villian Motive Power: Ennui Iteration
```{r}
...
```
## Villian Motive Power: Joy Iteration
```{r}
...
```
## Prior and motive impact on the posterior: Villian Motive Power
```{r}
# Motive Category 
crime <- stringr::str_to_title(motive_categories)
observation <- scooby_data %>% 
