...
<b>Fear</b> is not confident about the probability of unmasking, so $\pi$ can be set as  $0.50$. Considering Fear is not confident about his answer, the his distribution will have a wide shape, large variance. We picked $\alpha = 2$ and $\beta=2$  such that $\pi=0.50$ and $V[\pi]=0.50$, based on the beta distribution expectation: $E[\pi]= \frac{\alpha}{\alpha + \beta}$
</font>
...
## Villain Motive Theft: Fear Iteration
...
## Villain Motive Theft: Ennui Iteration
...
## Villain Motive Theft: Joy Iteration
...
## Prior and motive impact on the posterior: Villain Motive Theft
...
## Villain Motive Competition {background-color="beige"}

Summary of Competition Motive Episodes:
------

```{r}
scoob <- tidytuesdayR::tt_load("2021-07-13")
scoob <- as_tibble(data.frame(scoob[1]))


scoob1 <- scoob %>%
    filter(scoobydoo.motive == "Competition") %>%
    mutate(
        unmasked = ifelse(scoobydoo.unmask_fred == "TRUE" | scoobydoo.unmask_daphnie == "TRUE" | scoobydoo.unmask_velma == "TRUE" | scoobydoo.unmask_shaggy == "TRUE" | scoobydoo.unmask_scooby == "TRUE" | scoobydoo.unmask_other == "TRUE", "Unmasked_Yes", "Unmasked_No"),
        decade = case_when(
            year(scoobydoo.date_aired) < 1970 ~ "1960s",
            year(scoobydoo.date_aired) >= 1970 & year(scoobydoo.date_aired) < 1980 ~ "1970s",
            year(scoobydoo.date_aired) >= 1980 & year(scoobydoo.date_aired) < 1990 ~ "1980s",
            year(scoobydoo.date_aired) >= 1990 & year(scoobydoo.date_aired) < 2000 ~ "1990s",
            year(scoobydoo.date_aired) >= 2000 & year(scoobydoo.date_aired) < 2010 ~ "2000s",
            year(scoobydoo.date_aired) >= 2010 & year(scoobydoo.date_aired) < 2020 ~ "2010s",
            year(scoobydoo.date_aired) >= 2020 ~ "2020s"
        )
    )
```
...
episodes <- scoob1 %>%
    group_by(decade) %>%
    count(unmasked) %>%
    pivot_wider(names_from = unmasked, values_from = n) %>%
    replace(is.na(.), 0) %>%
    mutate(
        n_episodes = Unmasked_No + Unmasked_Yes,
    ) %>%
    ungroup()
...
generate_io_posteriors <- function(df, p_alpha, p_beta) {
    df$prior_alpha <- p_alpha
    df$prior_beta <- p_beta
    df <- df %>% dplyr::inner_join(episodes, by = "decade")
    for (i in 1:nrow(df)) {
        df$posterior_alpha[i] <- df$prior_alpha[i] + df$Unmasked_Yes[i]
        df$posterior_beta[i] <- df$prior_beta[i] + df$Unmasked_No[i]

        if (i < nrow(df)) {
            df$prior_alpha[i + 1] <- df$posterior_alpha[i]
            df$prior_beta[i + 1] <- df$posterior_beta[i]
        } else {}
    }
    df <- df[, -c(7, 8, 9)]
    return(df)
...
ennui <- generate_io_posteriors(ennui, Ennui_01prior_a, Ennui_01prior_b)
fear <- generate_io_posteriors(fear, fear_01prior_a, fear_01prior_b)
joy <- generate_io_posteriors(joy, joy_01prior_a, joy_01prior_b)
...
<font color="DarkGreen">Fear</font> Prior's and Posteriors for Competition Motive Episodes: {background-color="beige"}
------

```{r}
fear %>%
    mutate(prior = paste0("Beta(", prior_alpha, ",", prior_beta, ")"), posterior = paste0("Beta(", posterior_alpha, ",", posterior_beta, ")")) %>%
    select(decade, prior, posterior, emotion) %>%
    kable(booktabs = T) 
```

<font color="DarkBlue">Joy</font>  Prior's and Posteriors for Competition Motive Episodes: {background-color="beige"}
------
...
<font color="DarkRed">Ennui</font>  Prior's and Posteriors for Competition Motive Episodes: {background-color="beige"}
------
...
Evolution of the posteriors of all 3 emotions in Competition Motive Episodes over the decades: {background-color="beige"}
------

```{r}
all_results <- ennui %>% dplyr::bind_rows(list(joy, fear))
pie <- seq(0, 1, .05)
graph_data <- all_results %>% cross_join(data.frame(pie))

graph_data$f_of_pi <- dbeta(graph_data$pie, graph_data$posterior_alpha, graph_data$posterior_beta)

p <- ggplot(graph_data, aes(x = pie, y = f_of_pi, fill = emotion)) +
    geom_area(alpha = 0.4, position = "identity") +
    labs(title = "Posterior Beliefs of emotions after {closest_state} episodes", x = "pi", y = "f(pi)", legend = "Emotion", subtitle = "Modelling the probability that a Villain will be caught given that their motive was Competition", parse = TRUE) +
...
rm(list = c(
    "all_results", "ennui", "episodes", "fear", "graph_data", "joy",
    "scoob", "scoob1", "generate_io_posteriors"
))
```



## Villain Motive Power
...
# Filter df to motive categories and decade (1960s)
observation <- scooby_data %>% 
  filter(decade == decade_list[1],
         motive_cat == motive_categories)

# Season 1 Observations (1960s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 1960s
fear_69posterior_a <- summarize_beta_binomial(alpha = fear_01prior_a, beta = fear_01prior_b, y = obs_y, n = obs_n)[2,2]
fear_69posterior_b <- summarize_beta_binomial(alpha = fear_01prior_a, beta = fear_01prior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 60s
p_fear_69 <- plot_beta_binomial(alpha = fear_69posterior_a, beta = fear_69posterior_b) + 
  labs(title = "1960-1969") +  guides(fill = FALSE)

p_joy_69 <- plot_beta_binomial(alpha = joy_69posterior_a, beta = joy_69posterior_b) + 
  labs(title = "1960-1969") +  guides(fill = FALSE)

p_Ennui_69 <- plot_beta_binomial(alpha = Ennui_69posterior_a, beta = Ennui_69posterior_b) + 
  labs(title = "1960-1969") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter df to motive categories and decade (1970s)
observation <- scooby_data %>% 
  filter(decade == decade_list[2],
         motive_cat == motive_categories)

# Season 2 Observations (1970s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 1970s
fear_79posterior_a <- summarize_beta_binomial(alpha = fear_69posterior_a, beta = fear_69posterior_b, y = obs_y, n = obs_n)[2,2]
fear_79posterior_b <- summarize_beta_binomial(alpha = fear_69posterior_a, beta = fear_69posterior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 70s
p_fear_79 <- plot_beta_binomial(alpha = fear_79posterior_a, beta = fear_79posterior_b) + 
  labs(title = "1970-1979") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter df to motive categories and decade (1980s)
observation <- scooby_data %>% 
  filter(decade == decade_list[3],
         motive_cat == motive_categories)

# Season 3 Observations (1980s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 1980s
fear_89posterior_a <- summarize_beta_binomial(alpha = fear_79posterior_a, beta = fear_79posterior_b, y = obs_y, n = obs_n)[2,2]
fear_89posterior_b <- summarize_beta_binomial(alpha = fear_79posterior_a, beta = fear_79posterior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 80s
p_fear_89 <- plot_beta_binomial(alpha = fear_89posterior_a, beta = fear_89posterior_b) + 
  labs(title = "1980-1989") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter df to motive categories and decade (1990s)
observation <- scooby_data %>% 
  filter(decade == decade_list[4],
         motive_cat == motive_categories)

# Season 4 Observations (1990s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 1990s
fear_99posterior_a <- summarize_beta_binomial(alpha = fear_89posterior_a, beta = fear_89posterior_b, y = obs_y, n = obs_n)[2,2]
fear_99posterior_b <- summarize_beta_binomial(alpha = fear_89posterior_a, beta = fear_89posterior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 90s
p_fear_99 <- plot_beta_binomial(alpha = fear_99posterior_a, beta = fear_99posterior_b) + 
  labs(title = "1990-1999") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter df to motive categories and decade (2000s)
observation <- scooby_data %>% 
  filter(decade == decade_list[5],
         motive_cat == motive_categories)

# Season 5 Observations (2000s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 2000s
fear_09posterior_a <- summarize_beta_binomial(alpha = fear_99posterior_a, beta = fear_99posterior_b, y = obs_y, n = obs_n)[2,2]
fear_09posterior_b <- summarize_beta_binomial(alpha = fear_99posterior_a, beta = fear_99posterior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 00s
p_fear_09 <- plot_beta_binomial(alpha = fear_09posterior_a, beta = fear_09posterior_b) + 
  labs(title = "2000-2009") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter df to motive categories and decade (2010s)
observation <- scooby_data %>% 
  filter(decade == decade_list[6],
         motive_cat == motive_categories)

# Season 6 Observations (2010s)
obs_y = observation$unmasked %>% sum()
obs_n = observation$unmasked %>% length()

# Update Fear, Ennui, and Joy posteriors for 2010s
fear_19posterior_a <- summarize_beta_binomial(alpha = fear_09posterior_a, beta = fear_09posterior_b, y = obs_y, n = obs_n)[2,2]
fear_19posterior_b <- summarize_beta_binomial(alpha = fear_09posterior_a, beta = fear_09posterior_b, y = obs_y, n = obs_n)[2,3]
...
# Plot 10s
p_fear_19 <- plot_beta_binomial(alpha = fear_19posterior_a, beta = fear_19posterior_b) + 
  labs(title = "2010-2019") +  guides(fill = FALSE)
...
#| code-fold: true
#| 
# Filter the data for the 2020s 
observation <- scooby_data %>% 
  filter(decade == decade_list[7],  # 7th decade corresponds to the 2020s
         motive_cat == motive_categories)

# Calculate the number of successful unmaskings and total episodes for the 2020s
obs_y = observation$unmasked %>% sum()  # Total successful unmaskings (1's)
obs_n = observation$unmasked %>% length()  # Total number of episodes

# Update the posterior distributions for Fear, Ennui, and Joy for the 2020s
...
# Plot the updated posteriors for the 2020s (for each emotion)
...
#| code-fold: true
#| 
# Custom decade list including prior
decade_list2 <- append("prior", decade_list )
...
df_a_Ennui <- c(Ennui_01prior_a, 
                ...
                Ennui_19posterior_a,
                Ennui_29posterior_a)  
# posteriors have to be updated for each decade
...
df_Ennui <- tibble(alpha = df_a_Ennui,  
                   beta = df_b_Ennui, 
                   decade = decade_list2, 
                   name = "Ennui")
...
#| code-fold: true
#| 
# Set up the decade list, including the prior
decade_list2 <- append("prior", decade_list)
...
df_fear <- tibble(alpha = df_a_fear,  
                  beta = df_b_fear, 
                  decade = decade_list2, 
                  name = "Fear")
...
df_joy <- tibble(alpha = df_a_joy,  
                 beta = df_b_joy, 
                 decade = decade_list2, 
                 name = "Joy")

# Remove unused objects to clean up environment
rm(list = c("joy_29posterior_a", "joy_29posterior_b", 
            "Ennui_29posterior_a", "Ennui_29posterior_b", 
            "fear_29posterior_a", "fear_29posterior_b",
            "joy_19posterior_a", "joy_19posterior_b", 
            "Ennui_19posterior_a", "Ennui_19posterior_b", 
            "fear_19posterior_a", "fear_19posterior_b",
            "joy_09posterior_a", "joy_09posterior_b", 
            "Ennui_09posterior_a", "Ennui_09posterior_b", 
            "fear_09posterior_a", "fear_09posterior_b",
            "joy_99posterior_a", "joy_99posterior_b", 
            "Ennui_99posterior_a", "Ennui_99posterior_b", 
            "fear_99posterior_a", "fear_99posterior_b",
            "joy_89posterior_a", "joy_89posterior_b", 
            "Ennui_89posterior_a", "Ennui_89posterior_b", 
            "fear_89posterior_a", "fear_89posterior_b",
            "joy_79posterior_a", "joy_79posterior_b", 
            "Ennui_79posterior_a", "Ennui_79posterior_b", 
            "fear_79posterior_a", "fear_79posterior_b",
            "joy_69posterior_a", "joy_69posterior_b", 
            "Ennui_69posterior_a", "Ennui_69posterior_b", 
            "fear_69posterior_a", "fear_69posterior_b"))

```

## Villain Motive Power: Fear Iteration
```{r}
# Set the dataframe to df_fear
df <- df_fear
...
ggarrange(
  p_fear_prior + ylim(NA, yMax), 
  p_fear_Season_1 + ylim(NA, yMax), 
  p_fear_79 + ylim(NA, yMax), 
  p_fear_89 + ylim(NA, yMax), 
  p_fear_99 + ylim(NA, yMax), 
  p_fear_09 + ylim(NA, yMax), 
  p_fear_19 + ylim(NA, yMax), 
  p_fear_29 + ylim(NA, yMax),
  ncol = 4, nrow = 2
)
...
## Villain Motive Power: Ennui Iteration
```{r}
# Set the dataframe to df_Ennui
df <- df_Ennui
...
ggarrange(
  p_Ennui_prior + ylim(NA, yMax), 
  p_Ennui_Season_1 + ylim(NA, yMax), 
  p_Ennui_79 + ylim(NA, yMax), 
  p_Ennui_89 + ylim(NA, yMax), 
  p_Ennui_99 + ylim(NA, yMax), 
  p_Ennui_09 + ylim(NA, yMax), 
  p_Ennui_19 + ylim(NA, yMax), 
  p_Ennui_29 + ylim(NA, yMax),
  ncol = 4, nrow = 2
)
...
## Villain Motive Power: Joy Iteration
```{r}
# Set the dataframe to df_joy
df <- df_joy
...
ggarrange(
  p_joy_prior + ylim(NA, yMax), 
  p_joy_Season_1 + ylim(NA, yMax), 
  p_joy_79 + ylim(NA, yMax), 
  p_joy_89 + ylim(NA, yMax), 
  p_joy_99 + ylim(NA, yMax), 
  p_joy_09 + ylim(NA, yMax), 
  p_joy_19 + ylim(NA, yMax), 
  p_joy_29 + ylim(NA, yMax),
  ncol = 4, nrow = 2
)
...
## Prior and motive impact on the posterior: Villain Motive Power
```{r}
# Motive Category 
crime <- stringr::str_to_title(motive_categories)
observation <- scooby_data %>% 
...
# Calculate Posterior Means for Fear, Ennui, and Joy
mean_fear <- summarize_beta_binomial(alpha = fear_01prior_a, beta = fear_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)
mean_Ennui <- summarize_beta_binomial(alpha = Ennui_01prior_a, beta = Ennui_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)
mean_joy <- summarize_beta_binomial(alpha = joy_01prior_a, beta = joy_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)

# Update plots for Fear, Ennui, and Joy for the motive
p_fear_29 <- p_fear_29 + ylim(NA, yMax) + labs(title = paste("Posterior: ", crime)) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_fear))

p_Ennui_29 <- p_Ennui_29 + ylim(NA, yMax) + labs(title = paste("Posterior: ", crime)) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_Ennui))

p_joy_29 <- p_joy_29 + ylim(NA, yMax) + labs(title = paste("Posterior: ", crime)) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_joy))

# Observation for All Episodes (All Motives)
observation <- scooby_data

obs_y = observation$unmasked %>% sum()  
obs_n = observation$unmasked %>% length()  

# Calculate Posterior Means for Fear, Ennui, and Joy for all episodes
mean_fear <- summarize_beta_binomial(alpha = fear_01prior_a, beta = fear_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)
mean_Ennui <- summarize_beta_binomial(alpha = Ennui_01prior_a, beta = Ennui_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)
mean_joy <- summarize_beta_binomial(alpha = joy_01prior_a, beta = joy_01prior_b, y = obs_y, n = obs_n)[2,4] %>% round(2)

# Plot Posterior Distributions for All Episodes (All Crimes)
p_all_fear <- plot_beta_binomial(alpha = fear_01prior_a, beta = fear_01prior_b, y = obs_y, n = obs_n, likelihood = F, prior = F) +
  guides(fill = FALSE) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_fear)) + labs(title = "Posterior: All Crimes")

p_all_joy <- plot_beta_binomial(alpha = joy_01prior_a, beta = joy_01prior_b, y = obs_y, n = obs_n, likelihood = F, prior = F) +
  guides(fill = FALSE) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_joy)) + labs(title = "Posterior: All Crimes")

p_all_Ennui <- plot_beta_binomial(alpha = Ennui_01prior_a, beta = Ennui_01prior_b, y = obs_y, n = obs_n, likelihood = F, prior = F) + 
  guides(fill = FALSE) + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_Ennui)) + labs(title = "Posterior: All Crimes")

# Calculate Means for the Priors
mean_fear <- (fear_01prior_a / (fear_01prior_a + fear_01prior_b)) %>% round(2)
p_fear_prior <- p_fear_prior + ylim(NA, yMax) 
# + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_fear))

mean_Ennuo <- (Ennui_01prior_a / (Ennui_01prior_a + Ennui_01prior_b)) %>% round(2)
p_Ennui_prior <- p_Ennui_prior + ylim(NA, yMax) 
# + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_Ennuo))

mean_joy <- (joy_01prior_a / (joy_01prior_a + joy_01prior_b)) %>% round(2)
p_joy_prior <- p_joy_prior + ylim(NA, yMax) 
# + geom_text(x = 0.75, y = yMax * 0.75, label = paste("Mean=", mean_joy))
...
## Villain Motive Greed

## Fear

[Fear](fear.png){target="_blank"}


## Ennui

[Ennui](ennui.png){target="_blank"}


## Joy

[Joy](joy.png){target="_blank"}
